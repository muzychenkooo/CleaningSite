...
export const dryCleaningContent: ServiceContent = {
  ...
  forWhom: [
    ...
    'Регулярный уход за дорогой мебелью',
  ],
  process: defaultProcess,
  priceFrom: 'уточняйте по телефону',
...
const businessProcess: ProcessStep[] = [
  ...
  { title: 'Выполнение работ', description: 'Регулярное или разовое обслуживание по договору.' },
...
const businessFaq: FaqItem[] = [
  ...
  { question: 'Регулярное обслуживание или разово?', answer: 'И то и другое. Регулярность и объём работ закрепляем в договоре.' },
...
const businessIncludedBySlug: Record<string, string[]> = {
  office: ['Уборка офисных помещений', 'Мытьё полов и стёкол', 'Чистка санузлов и кухонных зон', 'Вынос мусора', 'Регулярность по графику'],
  ...
  mall: ['Уборка общих зон ТЦ/БЦ', 'Коридоры, лифты, санузлы', 'Регулярное обслуживание по графику'],
  fitness: ['Уборка залов, раздевалок, душевых', 'Обработка тренажёров и покрытий'],
  hotel: ['Уборка номеров и общих зон', 'Мытьё полов и стёкол', 'Регулярное обслуживание по стандартам отеля'],
  ...
  administrative: ['Уборка административных зданий', 'Офисные и общие зоны', 'Регулярность по договору'],
...
const businessForWhomBySlug: Record<string, string[]> = {
  ...
  warehouse: ['Склады и логистические центры', 'Регулярная или разовая уборка'],
  ...
  'residential-complex': ['ЖК и управляющие компании', 'Регулярная уборка подъездов'],
...
export function getBusinessServiceContent(slug: string): ServiceContent | null {
  ...
  return buildBusinessContent(
    business.label,
    'Регулярное обслуживание или разовые уборки. Договор, счёт, акты.',
    included,
    forWhom,
    related
  );
